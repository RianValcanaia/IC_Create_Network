...
class SlurmDeployGenerator:
    ...
    def deploy(self, time):
        ...
        # caminho do network.yaml relativo à raiz do projeto — no cluster o
        # projeto vive em cluster_project_dir, então o caminho relativo local
        # (base_dir -> network_yaml) é reaplicado sobre cluster_project_dir.
        # Necessário porque Definitivo exige -n/--network (diferente do
        # slurm/ original, onde era opcional).
        rel_network_yaml = self.paths.network_yaml.relative_to(self.paths.base_dir)
        cluster_network_yaml = f"{cluster_project_dir}/{rel_network_yaml}"
        ...
        script = self._build_script(
            ...
            num_nodes=num_nodes,
            time=time,
            network_yaml=cluster_network_yaml,
        )
    ...
    def _build_script(self, machines, coordinator, coord_node, project_dir,
                       log_dir, nodelist, num_nodes, time, network_yaml):
        """
        ...
        Todo comando 'python3 main.py' inclui '-n <network_yaml>' (obrigatório).
        """
        n_flag = f'-n {network_yaml}'
        ...
        for name, m in machines.items():
            ...
            lines.append(
                f'srun --nodes=1 --ntasks=1 --nodelist={node} '
                f'sg docker -c "python3 main.py {n_flag} --start --machine {name} --phase clean" &'
            )
        lines += [
            "wait",
            "",
            ...
            f'sg docker -c "python3 main.py {n_flag} --setup --phase clean"',
            "",
            "# ── Fase 0: Pré-requisitos (coordenador) ─────────────────────────",
            f'srun --nodes=1 --ntasks=1 --nodelist={coord_node} '
            f'sg docker -c "python3 main.py {n_flag} --setup --phase prereqs"',
            "",
        ...
        for name, m in machines.items():
            ...
            lines.append(
                f'srun --nodes=1 --ntasks=1 --nodelist={node} '
                f'sg docker -c "python3 main.py {n_flag} --start --machine {name} --phase cas" &'
            )
        lines += ["wait", ""]
        ...
        lines += [
            ...
            f'srun --nodes=1 --ntasks=1 --nodelist={coord_node} '
            f'sg docker -c "python3 main.py {n_flag} --setup --phase enroll"',
            "",
            "# ── Fase 3: Artefatos (coordenador) ─────────────────────────────",
            f'srun --nodes=1 --ntasks=1 --nodelist={coord_node} '
            f'sg docker -c "python3 main.py {n_flag} --setup --phase artifacts"',
            "",
        ...
        for name, m in machines.items():
            ...
            lines.append(
                f'srun --nodes=1 --ntasks=1 --nodelist={node} '
                f'sg docker -c "python3 main.py {n_flag} --start --machine {name} --phase nodes" &'
            )
        lines += ["wait", ""]
        ...
        lines += [
            ...
            f'srun --nodes=1 --ntasks=1 --nodelist={coord_node} '
            f'sg docker -c "python3 main.py {n_flag} --setup --phase channels"',
            "",
            "# ── Fase 6: Lifecycle do chaincode (coordenador) ─────────────────",
            f'srun --nodes=1 --ntasks=1 --nodelist={coord_node} '
            f'sg docker -c "python3 main.py {n_flag} --setup --phase chaincode"',
            "",
        ]
        ...
        if ccaas_machines:
            ...
            for name in ccaas_machines:
                ...
                lines.append(
                    f'srun --nodes=1 --ntasks=1 --nodelist={node} '
                    f'sg docker -c "python3 main.py {n_flag} --start --machine {name} --phase ccaas" &'
                )
            lines += ["wait", ""]
